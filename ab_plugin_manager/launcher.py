...
def launch_application(
    ...
    Вызов `launch_application` блокирует поток выполнения до завершения работы приложения.

    Любой исполняемый код приложения должен быть реализован в плагинах (некоторых из `core_plugins` или других плагинах,
    подгружаемых ими).

    Плагины ядра, как правило, включают:
    - плагин, осуществляющий загрузку дополнительных плагинов (обычно основанный на `PluginDiscoveryPlugin`)
    - плагин для управления конфигурацией (`ConfigPlugin`)
